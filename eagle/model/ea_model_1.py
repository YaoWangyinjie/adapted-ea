...
class EaModel(nn.Module):
    ...
    def setup_online_adaptation(
        ...
        if scope not in {"default", "head_only"}:
            raise ValueError(f"Unsupported adaptation scope: {scope}")
        if objective not in {"kl", "acceptance"}:
            ...
            for _ in range(train_steps):
                ...
                for fragment in fragments:
                    ...
                    if self.adaptation_objective == "acceptance":
                        ...
                        per_token = -draft_log_probs.gather(
                            -1, accepted_tokens.unsqueeze(-1)
                        ).squeeze(-1) * (temperature ** 2)
                        counts["accepted_positions"] = counts.get("accepted_positions", 0) + int(per_token.numel())
                    else:
                        per_token = F.kl_div(
                ...
                if token_loss_sum is None or valid_positions == 0:
                    ...
                    return result
                loss = token_loss_sum / valid_positions
                ...
                valid_parameters = all(
                    bool(torch.isfinite(parameter).all()) and self._fits_dtype(parameter, original_dtype)
                ...
                if not valid_parameters or not valid_optimizer:
                    ...
                    self.adapter_optimizer.load_state_dict(optimizer_snapshot)
                    self.adapter_optimizer.zero_grad(set_to_none=True)
                    reason = "nonfinite_optimizer_state" if not valid_optimizer else "invalid_updated_parameters"
