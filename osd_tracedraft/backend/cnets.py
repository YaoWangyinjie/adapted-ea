...
class LlamaRMSNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # Preserve a FP32 output-norm weight without promoting the head input.
        # Rounding the normalized activation first retains the legacy path when
        # the norm still has its checkpoint value; small norm updates survive
        # until multiplication instead of being rounded away at publication.
        normalized = hidden_states.to(input_dtype)
        if self.weight.dtype == torch.float32 and input_dtype != torch.float32:
            return (self.weight * normalized.float()).to(input_dtype)
